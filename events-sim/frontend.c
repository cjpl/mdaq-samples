...
INT begin_of_run(INT run_number, char *error)
{
   ...
   if(evt == NULL) {
     printf("Error! No event generator!\n");
   } else
     printf("Event generator: 0x%08x\n", (u_int32_t)evt);
...
INT read_trigger_event(char *pevent, INT off)
{
   ...
   /* for (a = 0; a < N_ADC; a++) */
   /*    *pdata++ = rand() % 1024; */
   c_gen_event(evt, pdata, N_ADC, 4096);
   pdata += N_ADC;
   ...
   /* for (a = 0; a < N_TDC; a++)
          *pdata++ = rand() % 1024; */
   c_gen_event(evt, pdata, N_TDC, 4096);
   pdata += N_TDC;
